      ...
      function makeResponseLogger(suffix){
        return function(response){
          ...
          if (fields.ua && fields.uid)
            (lastAccessPerUA[fields.ua] = lastAccessPerUA[fields.ua] || {})[fields.uid] = startDate;
    ...
    for (var i = 0; route = routes[i]; i++) {
      ...
      if (routeMatch) {
        ...
        prepareResponse(self, request, response, function(request, response){
          if (!route.controller)
            console.error("controller not found", route.hasQuery ? request.url : path, route.pattern);
          else
