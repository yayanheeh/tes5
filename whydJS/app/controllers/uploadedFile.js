...
exports.config.uploadPath = exports.config.whydPath + exports.config.uploadSubDir;
exports.config.uAvatarImgPath = exports.config.whydPath + exports.config.uAvatarImgDir;
exports.config.uCoverImgPath = exports.config.whydPath + exports.config.uCoverImgDir;
exports.config.uPlaylistPath = exports.config.whydPath + exports.config.uPlaylistDir;
...
exports.actualFilePath = function (filepath) {
	if (!filepath || filepath.indexOf(exports.config.whydPath) == 0)
		...
		return filepath = exports.config.whydPath + (filepath[0] != "/" ? "/" : "") + filepath;
}
...
exports.renameTo = function (filename, toFilename, callback) {
	...
	if (!filename || !toFilename)
		return error("invalid filename or toFilename: " + filename + ", " + toFilename);
	try {
...
exports.moveTo = function (filename, toPath, callback) {
	...
	if (!filename || !toPath) {
		if (callback)
			callback();
		console.log("ERROR: invalid filename or toPath: " + filename + ", " + toPath);
		return filename;
	}
...
exports.controller = function(request, reqParams, response) {
	...
	function renderNoImage() {
		response.renderFile(exports.config.whydPath + "/public/images/no_image.png");
	}
	...
	function renderFile(path, defaultImg) {
		...
		response.renderFile(""+path, null, null, function(error) {
			...
			if (defaultImg)
				response.renderFile(exports.config.whydPath + "/public" + defaultImg, null, null, renderNoImage);
			else
				renderNoImage();
		});
	}
	...
	function renderImg(uri, defaultImg) {
		if (uri && (""+uri).indexOf("//") != -1) {
			...
			response.temporaryRedirect(uri.replace("http:", "") + (args > -1 ? request.url.substr(args) : ""));
		}
		else
			renderFile(uri, defaultImg);
	}
	...
	function renderUserImg(id) {
		...
		if (user && user.img) {
			...
			var args = request.url.indexOf("?");
			response.temporaryRedirect(user.img.replace("http:", "") + (args > -1 ? request.url.substr(args) : ""));
		}
		else
			renderFile(exports.config.uAvatarImgPath + "/" + id, "/images/blank_user.gif");
	}
	...
	var renderTypedImg = {
		...
		"userCover": function(id) {
			if (id.indexOf(".") > -1)
				return renderFile(exports.config.uCoverImgPath + "/" + id, "/images/1x1-pixel.png");
			userModel.fetchByUid(id, function(user){
				if (user && user.cvrImg) {
					var args = request.url.indexOf("?");
					response.temporaryRedirect(user.cvrImg + (args > -1 ? request.url.substr(args) : ""));
				}
				else
					renderFile(exports.config.uCoverImgPath + "/" + id, "/images/1x1-pixel.png");
			});
		},
		"post": function(id) {
		...
		"playlist": function(id, reqParams) {
			...
			function renderLastPostImg() {
				...
				postModel.fetchPlaylistPosts(parts[0], parts[1], {limit:1}, function(posts) {
					for (var i in posts) {
						var img = (posts[i] || {}).img;
						if (img) {
							renderImg(img);
			...
			if (reqParams.remoteOnly)
				fs.stat(filePath, function(error, stats) {
					if (error || !stats.isFile())
						...
						renderImg(config.urlPrefix + "/images/1x1-pixel.png"); // transparent image
				});
			else
	...
	if (reqParams.id) {
		if (reqParams.type && renderTypedImg[reqParams.type])
			...
			renderFile(exports.config.uploadPath + "/" + reqParams.id);
	}
	else if (reqParams.uAvatarImg)
		renderTypedImg["user"](reqParams.uAvatarImg);
